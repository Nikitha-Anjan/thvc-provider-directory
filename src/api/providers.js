// Simulated API (no delay)
// Exports fetchProviders and fetchProvider (both return Promise)

export const fetchProviders = () => {
  return Promise.resolve(fullDataset.map(makePartialRepresentation));
};

export const fetchProvider = (providerId) => {
  const provider = fullDataset.find(p => p.id === providerId);
  return provider ? Promise.resolve(provider) : Promise.reject(new Error('Provider not found'));
};
